...
import { POST } from "@/app/api/jobs/full-book-generation/route";
import { GET } from "@/app/api/jobs/full-book-generation/[jobId]/route";
import {
...
describe("full-book generation job route", () => {
  ...
  it("returns the full-book generation job for the active workspace", async () => {
    ...
    const response = await GET(
      new Request(`http://localhost/api/jobs/full-book-generation/${job?.id}` as string, {
        headers: {
...
describe("full-book generation enqueue route", () => {
  const createdDirs: string[] = [];
  ...
  async function postFullBook(input: {
    workspaceId: string;
    body: Record<string, unknown>;
  }) {
    return POST(
      new Request("http://localhost/api/jobs/full-book-generation", {
        method: "POST",
        headers: {
  ...
  it("ignores client metadata without requiring a legacy account session", async () => {
    useTemporaryDatabase();
    seedStoredBook("workspace-jobs");

    const response = await postFullBook({
      workspaceId: "workspace-jobs",
      body: {
        bookId: " BOOK-1 ",
        chapterCount: 999_999,
        narratorId: "uploaded-clone",
        mode: "immersive",
      },
    });
    const payload = (await response.json()) as {
    ...
    expect(getGenerationJob(payload.job?.id ?? "", "workspace-jobs")).toMatchObject({
      workspaceId: "workspace-jobs",
      bookId: "book-1",
      narratorId: "marlowe",
      mode: "classic",
  ...
  it("uses the server-owned current sample voice for the full audiobook", async () => {
    useTemporaryDatabase();
    seedStoredBook("workspace-jobs");
    ...
    expect(
      completeGenerationJob(sampleJob?.id ?? "", "workspace-jobs", {
        assetPath: "generated/workspace-jobs/book-1/sample-jules.wav",
        mimeType: "audio/wav",
        provider: "mock",
      }),
    ...
    expect(response.status).toBe(201);
    await expect(response.json()).resolves.toMatchObject({
      job: { narratorId: "jules", mode: "classic" },
    });
  });

  it("uses the server-owned default voice when no current sample exists", async () => {
    useTemporaryDatabase();
    seedStoredBook("workspace-jobs");
    ...
    expect(response.status).toBe(201);
    await expect(response.json()).resolves.toMatchObject({
      job: { narratorId: "marlowe", mode: "classic" },
    });
  });
  ...
  it("rejects an unknown book without creating a job", async () => {
    ...
    const response = await postFullBook({
      workspaceId: "workspace-jobs",
      body: { bookId: "book-missing", chapterCount: 1 },
    });
  ...
  it("rejects a book from another workspace without leaking or creating data", async () => {
    ...
    const response = await postFullBook({
      ...
      body: { bookId: "book-1", chapterCount: 1 },
    });
  ...
  it.each([
    ...
    {
      label: "excessive stored chapter count",
      chapterCount: 301,
      text: "Chapter 1\nStored text",
  ...
  it("rejects a duplicate active full-book job without creating another job", async () => {
    useTemporaryDatabase();
    seedStoredBook("workspace-jobs");
