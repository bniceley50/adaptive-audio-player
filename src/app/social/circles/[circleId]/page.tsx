...
import { getPublicCircleDetail, formatSocialCommunityEventLabel } from "@/features/social/public-social";
    ...
    <AppShell eyebrow="Book circle" title={detail.circle.title}>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-3xl font-semibold tracking-tight text-stone-950">
              {detail.circle.bookTitle}
            </h2>
            <p className="mt-3 text-base leading-7 text-stone-600">{detail.circle.summary}</p>
            <p className="mt-3 text-sm leading-6 text-stone-600">
              {detail.circle.vibe}. Hosted by {detail.circle.host}.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link className="rounded-full bg-stone-950 px-5 py-3 text-sm font-medium text-white transition hover:bg-stone-800" href={`/import?edition=${detail.circle.editionId}`}>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <article className="rounded-[1.4rem] border border-stone-200 bg-white p-5 shadow-sm">
          <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-stone-500">Joins</p>
          <p className="mt-3 text-2xl font-semibold text-stone-950">{detail.summary?.joins ?? 0}</p>
        </article>
        <article className="rounded-[1.4rem] border border-stone-200 bg-white p-5 shadow-sm">
          <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-stone-500">Reopens</p>
          <p className="mt-3 text-2xl font-semibold text-stone-950">{detail.summary?.reopens ?? 0}</p>
        </article>
        <article className="rounded-[1.4rem] border border-stone-200 bg-white p-5 shadow-sm">
          <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-stone-500">Shares</p>
          <p className="mt-3 text-2xl font-semibold text-stone-950">{detail.summary?.shares ?? 0}</p>
        </article>
        <article className="rounded-[1.4rem] border border-stone-200 bg-white p-5 shadow-sm">
          <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-stone-500">Checkpoint</p>
          <p className="mt-3 text-sm font-semibold text-stone-950">{detail.circle.checkpoint}</p>
        </article>
      </section>
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-[0.22em] text-stone-500">Recommended edition</p>
          <div className="mt-4 rounded-[1.2rem] border border-stone-200 bg-stone-50/80 p-4">
            <p className="text-sm font-semibold text-stone-950">{detail.edition?.title ?? "Edition unavailable"}</p>
            <p className="mt-2 text-sm leading-6 text-stone-600">
              {detail.edition
                ? `${detail.edition.narratorName} in ${detail.edition.mode} for ${detail.edition.bookTitle}`
                : "This circle does not have a linked edition yet."}
            </p>
            {detail.edition ? (
              <Link className="mt-3 inline-flex rounded-full border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700 transition hover:border-stone-400 hover:bg-stone-50" href={`/social/editions/${detail.edition.id}`}>
                View edition
              </Link>
            ) : null}
          </div>
